...
│  │ Middleware│→│ Limiter  │→│ (format detect)  │  │
│  └──────────┘  └──────────┘  └────────┬─────────┘  │
│                                        │            │
│  ┌─────────────────────────────────────┘            │
│  │                                                  │
...
6. **Rate limit check** → Sliding window counter in Redis → waits if needed
7. **LLM call** → Forwards to NVIDIA/OpenAI with enriched context
...
Abstract `LLMProvider` base class with `chat()` and `chat_stream()` methods. Adding a new provider (Anthropic, local models, etc.) requires only implementing the interface.
...
### Rate Limiter
Atomic sliding window implemented via Redis Lua script. Tracks request timestamps in a sorted set, removes expired entries, and atomically checks/increments in a single Redis roundtrip.
