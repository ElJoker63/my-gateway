"""
Ollama Cloud / Remote LLM provider.
Uses Ollama's OpenAI-compatible API endpoint for cloud or remote instances.
Supports per-request API key authentication and multi-key pool rotation.
"""
...
class OllamaProvider(LLMProvider):
    ...
    async def chat(
        ...
        **kwargs,
        ...
        logger.debug(f"Ollama Cloud chat request: model={params['model']}, messages={len(messages)}")

        headers = self._get_auth_headers(resolved_key) if resolved_key else {}
    ...
    async def chat_stream(
        ...
        **kwargs,
        ...
        logger.debug(f"Ollama Cloud stream request: model={params['model']}, messages={len(messages)}")
    ...
    async def health_check(self) -> bool:
        """Check if Ollama Cloud service is reachable."""
        key = self.api_key
        if not key:
            from app.services.key_manager import key_manager
            key = key_manager.get_any_key("ollama")

        headers = self._get_auth_headers(key) if key else {}
        try:
            client = await self._get_client()
